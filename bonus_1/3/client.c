...
void send_reversed(char* response, int client_socket){
	reverse_string(response);
	printf("Reversed string: %s. Bytes: %ld\n", response, strlen(response));
	...
	for(;;){
        ...
        if (FD_ISSET(client_socket, &readfds)) {
            ...
            if (return_value > 0) {
				...
				send_reversed(buffer, client_socket);
            ...
            printf("No server response. Quit...\n");
			break;
        }
    }
