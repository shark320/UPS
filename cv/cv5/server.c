		...
		if (client_socket > 0)
		{
			...
			if (return_value == 0)
			{
				...
				for(;;){
					...
					int prijato = 0;
					while (prijato<10){
						int num = recv(client_socket, cbuf + prijato, 10-prijato, 0);
						if (num<=0){
							break;
						}
						prijato += num;
					...
					if (strncmp(cbuf, "KIVUPS", 6) != 0){
						printf("Error hedader");
					}

					int delka = atoi(cbuf + 6);
					printf("Length: %d\n", delka);

					prijato = 0;
					memset(cbuf, 0 ,51);
					while (prijato<delka){
						int num = recv(client_socket, cbuf + prijato, delka-prijato, 0);
						if (num<=0){
							break;
						}
						prijato += num;
				...
				}; // dokud neco na vstupu je a dokud klient komunikuje
