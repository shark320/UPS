...
bool header::check_values() const {
    return length >= 0 && length <= constants::MSG_MAX_LENGTH && _status != status::NULL_STATUS &&
           _subtype != subtype::NULL_SUBTYPE && _type != type::NULL_TYPE;
}
...
std::shared_ptr<header> header::extract(const std::string &message) {
    ...
    std::string msg_identifier = message.substr(constants::MSG_IDENTIFIER_FIELD_POS,
                                                constants::MSG_IDENTIFIER_FIELD_LENGTH);
    ...
    int msg_subtype_int = std::stoi(
            message.substr(constants::MSG_SUBTYPE_FIELD_POS, constants::MSG_SUBTYPE_FIELD_LENGTH));
...
std::string header::to_string() const {
    return fmt::format("Header [identifier={}, type={}, subtype={}, status={}, length={}]", identifier,
                       type_mapper::get_id(_type), subtype_mapper::get_id(_subtype), status_mapper::get_id(_status),
                       this->length);
}

header::header(const std::shared_ptr<header> &other) : header(
        other->identifier,
        other->_type,
        other->_subtype,
        other->_status
